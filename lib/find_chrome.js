...
function linux(canary) {
  ...
  // Look for google-chrome(-stable) executables by using the which command
  const executables = [
  ...
  if (!installations.length)
    throw new Error('The environment variable CHROME_PATH must be set to a local installation of Google Chrome.');
...
function findChromeExecutables(folder) {
  ...
  const chromeExecRegex = '^Exec=\/.*\/(google-chrome|chrome)-.*';
